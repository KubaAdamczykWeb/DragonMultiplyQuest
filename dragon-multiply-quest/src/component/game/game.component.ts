...
export class GameComponent {
  ...
  public gameResultTable: GameResult[] = [];
  ...
  private finishTask(): void {
    this.timestampId = setTimeout(()=>{
      ...
      let points = 10 - duration - punishment + this.rightAnswer!.toString().length;
      if(points > 10){
         points = 10;
      } else if (points <= 0 && punishment === 0){
         points = 1;
      } else if(points < 0){
        points = 0;
      }
      ...
      this.gameResultTable.unshift({
        task: `${task.x} x ${task.x} = ${this.rightAnswer}`,
        points: points
      })
